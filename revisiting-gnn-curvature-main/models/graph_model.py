...
from torch.nn import ModuleList, Dropout, ReLU, Sigmoid, LeakyReLU
    ...
    def __init__(self, in_features, out_features, num_relations):
        ...
        for i in range(self.num_relations):
            convs.append(GINConv(
                nn.Sequential(nn.Linear(in_features, out_features), nn.BatchNorm1d(out_features), nn.ReLU(),
...
class GNN(torch.nn.Module):
        ...
        if self.args.last_layer_fa:
            if self.layer_type == "R-GCN" or self.layer_type == "GCN":
                ...
                self.last_layer_transform = nn.Sequential(nn.Linear(self.args.hidden_dim, self.args.hidden_dim),
                                                          nn.BatchNorm1d(self.args.hidden_dim), nn.ReLU(),
    ...
    def get_layer(self, in_features, out_features):
        if self.layer_type == "GCN":
            ...
            return GINConv(nn.Sequential(nn.Linear(in_features, out_features), nn.BatchNorm1d(out_features), nn.ReLU(),
                                         nn.Linear(out_features, out_features)))
...
class GCN_edge_weight(torch.nn.Module):
    def __init__(self, args):
        super(GCN_edge_weight, self).__init__()
        print('Edge weight with GCNConv is used')
        self.args = args
        self.num_relations = args.num_relations
        self.layer_type = args.layer_type
        num_features = [args.input_dim] + list(args.hidden_layers) + [args.output_dim]  # prev
        # num_features = [args.input_dim] + list(args.hidden_layers) + [args.hidden_dim]
        self.num_layers = len(num_features) - 1
        layers = []
        for i, (in_features, out_features) in enumerate(zip(num_features[:-1], num_features[1:])):
            layers.append(self.get_layer(in_features, out_features))
        ...
        # self.act_fn = Sigmoid()
        # self.lin_head = nn.Linear(args.hidden_dim, args.output_dim)
    ...
    def forward(self, graph, measure_dirichlet=False):
        ...
        # print(x.shape)  # TODO: this is no node features for x!!!
        x = x.float()
        edge_weight = edge_weight.float()
        for i, layer in enumerate(self.layers):
            # there are (self.num_layers - 1) conv layers, with idx 0, 1, ..., (self.num_layers - 2)
            x_new = layer(x, edge_index, edge_weight=edge_weight)
            # x_new = layer(x, edge_index)  # TODO: problem: the output of the first GCNConv layer is already zero!!!
            if i != self.num_layers - 1:
                if i != self.num_layers - 2:  # not the last layer, apply relu
                    # print(x_new)
                    x_new = self.act_fn(x_new)
                    # print(x_new)
                # exit()
