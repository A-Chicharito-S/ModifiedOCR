...
from models.graph_model import GNN, GCN_edge_weight
...
class Experiment:
    def __init__(self, args=None, dataset=None, train_dataset=None, validation_dataset=None, test_dataset=None):
        ...
        self.model = GNN(self.args).to(self.args.device) if not args.use_edge_weight else GCN_edge_weight(self.args).to(
            self.args.device)
        ...
        if self.test_dataset is None:
            ...
            self.args.train_data, self.args.validation_data = random_split(self.args.train_data,
                                                                           [train_size, validation_size])
    ...
    def run(self):
        ...
        for idx, graph in enumerate(train_loader):
            # print(graph.y)
            y_eq_1 += graph.y.sum()
        print('training set y label=1: {}'.format(y_eq_1/len(train_loader.dataset)))

        y_eq_1 = 0
        for idx, graph in enumerate(validation_loader):
            y_eq_1 += graph.y.sum()
        print('val set y label=1: {}'.format(y_eq_1 / len(validation_loader.dataset)))

        y_eq_1 = 0
        for idx, graph in enumerate(test_loader):
            y_eq_1 += graph.y.sum()
        print('test set y label=1: {}'.format(y_eq_1/len(test_loader.dataset)))
        # exit()
        for epoch in range(1, 1 + self.args.max_epochs):
            ...
            optimizer.zero_grad()
            # print(self.model)
            # test_para = self.model.layers[1].lin.weight.detach().clone()
            ...
            for graph in train_loader:
                ...
                optimizer.step()
                optimizer.zero_grad()
            # assert not torch.equal(test_para, self.model.layers[1].lin.weight)
            # print(torch.equal(test_para, self.model.layers[1].lin.weight))
            # meaning it is updated (compared with the prev epoch)
